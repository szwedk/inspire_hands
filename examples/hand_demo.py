...
def parse_args():
    ...
    parser.add_argument(
        "--demo", "-d",
        type=str,
        choices=["basic", "gestures", "fingers", "wave", "count", "crawl"],
        default="basic",
...
def crawl_demo(hand):
    print("\nRunning crawling demo...")

    hand.reset()
    time.sleep(1)
    hand.set_all_finger_speeds(1000)

    for _ in range(6):  # Adjust number of crawl cycles
        # Phase 1: Rear fingers lift (ring, little), front supports (thumb, index, middle)
        print("Phase 1: Lift rear, support front")
        ...
        # Phase 2: Rear fingers push forward
        print("Phase 2: Push rear forward")
        hand.ring_finger.close()
        hand.little_finger.close()
        time.sleep(0.5)

        # Phase 3: Front fingers lift (thumb, index, middle), rear supports
        print("Phase 3: Lift front, support rear")
        hand.ring_finger.close()
        hand.little_finger.close()
        hand.thumb_bend.open()
        hand.index_finger.open()
        ...
        # Phase 4: Pull forward with front fingers
        print("Phase 4: Pull front forward")
        hand.thumb_bend.close()
        hand.index_finger.close()
        hand.middle_finger.close()
        time.sleep(0.5)
...
def main():
    ...
    hand = None
    try:
        hand = InspireHand(port=args.port, baudrate=args.baudrate)
        hand.open()
